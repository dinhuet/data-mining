...
# --- HÀM HỖ TRỢ CHUYỂN DEVICE (Giữ nguyên) ---
...
class Config(object):
    def __init__(self, args):
        ...
        self.dis2idx = np.zeros((512), dtype='int64')  # Sửa np.int64 cho đúng chuẩn numpy mới
        self.dis2idx[1] = 1
        self.dis2idx[2:] = 2
        self.dis2idx[4:] = 3
        self.dis2idx[8:] = 4
        ...
        wandb.init(name=wandb_name, project="meta", config={"batch_size": self.batch_size,
                                                            "epoch": self.max_epoch, "lr": self.lr,
                                                            ...
                                                            "type_path_num": self.type_path_num,
                                                            "load_data_type": self.load_data_type,
                                                            "max_grad_norm": self.max_grad_norm,
                                                            "graph_out_hidden": self.graph_out_hidden,
                                                            "word_embed_hidden": self.word_embed_hidden,
                                                            "entity_type_size": self.entity_type_size,
                                                            ...
                                                            "seq_loss_weight": self.seq_loss_weight,
                                                            "train_data": self.train_prefix,
                                                            "test_data": self.test_prefix})
        ...
        # [QUAN TRỌNG] Định nghĩa Device ngay tại Init
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        print(f"--- CONFIG: Đang chạy trên thiết bị: {self.device} ---")
    ...
    def load_train_data(self):
        ...
        self.train_load = Data.DataLoader(self.train_data, batch_size=self.batch_size,
                                          shuffle=shuffle, sampler=None, batch_sampler=None, num_workers=0,
                                          collate_fn=self.train_data.get_batch,
    ...
    def load_test_data(self):
        ...
        self.test_load = Data.DataLoader(self.test_data, batch_size=self.batch_size,
                                         shuffle=shuffle, sampler=None, batch_sampler=None, num_workers=0,
                                         collate_fn=self.test_data.get_batch,
                                         pin_memory=False, drop_last=False, timeout=0, worker_init_fn=None)
    ...
    def criterion(self, inputs, output, labels):
        ...
        seq_pred = mask_pred.view(-1, mask_pred.shape[-1])[
            torch.arange(output["seq_truth"].view(-1).shape[0]), output["seq_truth"].view(-1)].view(
            output["seq_truth"].shape)
    ...
    def interface(self, inputs, output, labels, loss_dict, type="train"):
        ...
        seq_pred = seq_pred.view(-1, seq_pred.shape[-1])[
            torch.arange(output["seq_truth"].view(-1).shape[0]), output["seq_truth"].view(-1)].view(
            output["seq_truth"].shape)
    ...
    def get_adj_mask(self, inputs, output):
        ...
        # [QUAN TRỌNG] Thay thế .cuda() bằng .to(self.device)
        adj_ext = torch.ones(adj.shape[0], adj.shape[1] + 1, adj.shape[2] + 1).to(self.device)
        adj_ext[:, 1:, 1:] = adj

        # [QUAN TRỌNG] Thay thế .cuda() bằng .to(self.device)
        startnode = torch.zeros_like(output["seq_truth"][..., 0]).to(self.device).unsqueeze(-1)
    ...
    def train(self, model_pattern, model_name):
        ...
        for epoch in range(self.max_epoch):
            ...
            for inputs, labels in self.train_load:
                ...
                # [QUAN TRỌNG] Đẩy dữ liệu vào GPU bằng self.device
    ...
    def test(self, model, model_name, epoch=0, global_step=0, gen_output=False, send_data=True):
        ...
        for inputs, labels in self.test_load:
            with torch.no_grad():
                ...
                # [QUAN TRỌNG] Đẩy dữ liệu Test vào GPU
                inputs = move_to_device(inputs, self.device)
                labels = move_to_device(labels, self.device)
                            ...
                            for t_idx in range(L_vertex[i]):
                                ...
                                for label_idx in range(1, self.relation_num):
                                    if pre_re_label[i, h_idx, t_idx, label_idx] > 0:
                                        test_result.append({"title": titles[i], "h_idx": h_idx, "t_idx": t_idx,
                                                            "r": self.id2rel[label_idx]})
        ...
        for loss_type, loss_ins in self.test_metric.items():
            if loss_type == "Re":
                test_acc_re, test_recall_re, test_f1_re, theta = loss_ins.cal_roc_metric(global_step,
                                                                                         send_data=send_data)
            else:
    ...
    def testall(self, model_pattern, model_name):  # , ignore_input_theta):
        ...
        # [QUAN TRỌNG] Sửa .cuda() thành .to(self.device)
        ...
        test_acc_re, test_recall_re, test_f1_re, theta = self.test(model, model_name, gen_output=True,
                                                                   send_data=False)
        self.logger.logging(
            "Test Re Acc:{:8.3f}Recall:{:8.3f}F1:{:8.3f}".format(test_acc_re, test_recall_re, test_f1_re))
...
class Metrics():
    ...
    def cal_bin_data(self, predict, label, mask):
        acc = torch.sum(
            (torch.gt(label, 0) & torch.eq(predict, label.long())).float() * mask)  # torch.gt(output_exist,0)
    ...
    def cal_metric(self, global_step, log=True, send_data=True):
        ...
        if log:
            self.logger.logging(
                "{:15} Loss{:8.3f} Predict:{:8d}Total:{:8d}True:{:8d} Acc:{:8.3f}%Recall:{:8.3f}%F1:{:8.3f}%".format(
                    self.prefix, loss, int(self.total_predict), int(self.total_truth), int(self.total_acc), 100 * acc,
                    100 * recall, 100 * f1))
    ...
    def cal_roc_metric(self, global_step, log=True, send_data=True):
        ...
        if self.res1 == []:
            ...
            theta, acc, recall, f1 = roc_cal(rel_res, prefix=self.prefix, his_acc=correct, his_pre=predict,
                                             his_recall=recall)
        loss = self.total_loss / self.batch_cnt
        if send_data:
            wandb.log({self.prefix + 'accuracy': acc,
        ...
        if log:
            self.logger.logging(
                "{:15} Loss{:8.3f} Theta:{:8.3f}{:29} Acc:{:8.3f}%Recall:{:8.3f}%F1:{:8.3f}%".format(self.prefix, loss,
                                                                                                     theta, " ",
                                                                                                     100 * acc,
                                                                                                     100 * recall,
                                                                                                     100 * f1))
